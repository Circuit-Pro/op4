...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    if UseLQR:
      ...
      ret.lateralTuning.lqr.ki = 0.21
      ret.lateralTuning.lqr.dcGain = 0.012
 ...
 #Tune To base Kona EV tune off of.
  ...
  def update(self, c, can_strings):
    ...
    if Params().get_bool('spasEnabled'):
      if self.CS.mdps11_stat == 7 and not self.CC.lkas_active and not self.CC.spas_active and not self.CC.turning_indicator_alert:
        events.add(EventName.buttonCancel)

      if not self.CC.lkas_active and self.CS.mdps11_stat == 7 and self.CC.spas_active and not self.CC.turning_indicator_alert: # We need to alert driver when SPAS abort or fail.
        events.add(EventName.steerSaturated)
